...
import com.rbkmoney.fistfulreporter.domain.enums.ChallengeEventType;
...
public class IdentityChallengeStatusChangedHandler implements IdentityEventHandler {
    ...
    private void updateChallenge(SinkEvent event, ChallengeChange challengeChange, ChallengeStatus status) throws DaoException {
        ...
        challenge.setChallengeStatus(TBaseUtil.unionFieldToEnum(status, com.rbkmoney.fistfulreporter.domain.enums.ChallengeStatus.class));
        if (status.isSetCompleted()) {
            ChallengeCompleted challengeCompleted = status.getCompleted();
            challenge.setChallengeResolution(TypeUtil.toEnumField(challengeCompleted.getResolution().toString(), com.rbkmoney.fistfulreporter.domain.enums.ChallengeResolution.class));
